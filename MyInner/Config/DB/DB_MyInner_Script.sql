...
-- -----------------------------------------------------
-- procedure SP_AVG_ASSESSMENTS
-- -----------------------------------------------------

DELIMITER $$
USE `db_myinner`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_AVG_ASSESSMENTS`(ID_USER INT (11),TYPE_ASPECT VARCHAR(15)) 
BEGIN

IF ( TYPE_ASPECT = 'ETICA') THEN
SELECT ROUND((SELECT AVG(ETICA_VALUE) FROM TBL_ASSESSMENTS AS AV WHERE AV.ID_USER_EVALUATED = ID_USER));

ELSEIF (TYPE_ASPECT = 'MORAL') THEN
SELECT ROUND((SELECT AVG(MORAL_VALUE) FROM TBL_ASSESSMENTS AS AV WHERE AV.ID_USER_EVALUATED = ID_USER));

ELSEIF (TYPE_ASPECT = 'HONESTIDADE') THEN
SELECT ROUND((SELECT AVG(HONESTIDADE_VALUE) FROM TBL_ASSESSMENTS AS AV WHERE AV.ID_USER_EVALUATED = ID_USER));

ELSEIF (TYPE_ASPECT = 'INTELIGENCIA') THEN
SELECT ROUND((SELECT AVG(INTELIGENCIA_VALUE) FROM TBL_ASSESSMENTS AS AV WHERE AV.ID_USER_EVALUATED = ID_USER));

ELSEIF (TYPE_ASPECT = 'RESPONSABILIDADE') THEN
SELECT ROUND((SELECT AVG(RESPONSABILIDADE_VALUE) FROM TBL_ASSESSMENTS AS AV WHERE AV.ID_USER_EVALUATED = ID_USER));
...
-- --------------------------------------------------------------------------------------------------------------------------------------
-- procedure SP_INSERT_ASSESSMENTS
-- --------------------------------------------------------------------------------------------------------------------------------------

DELIMITER $$
USE `db_myinner`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_INSERT_ASSESSMENTS`(_ID_USER_EVALUATOR BIGINT(20),_ID_USER_EVALUATED BIGINT(20),_ETICA_VALUE TINYINT(4),_MORAL_VALUE TINYINT(4),
_HONESTIDADE_VALUE TINYINT(4),_INTELIGENCIA_VALUE TINYINT(4),_RESPONSABILIDADE_VALUE TINYINT(4),_TEXT_INFORMATION TEXT) 
BEGIN
  INSERT INTO TBL_ASSESSMENTS(ID_USER_EVALUATOR,ID_USER_EVALUATED ,ETICA_VALUE ,MORAL_VALUE ,HONESTIDADE_VALUE ,INTELIGENCIA_VALUE ,RESPONSABILIDADE_VALUE ,TEXT_INFORMATION)
  VALUES (_ID_USER_EVALUATOR ,_ID_USER_EVALUATED ,_ETICA_VALUE ,_MORAL_VALUE ,_HONESTIDADE_VALUE ,_INTELIGENCIA_VALUE ,_RESPONSABILIDADE_VALUE ,_TEXT_INFORMATION);
END$$
...
WHERE TBL_LOGIN.PASSWORD_USER = _PASSWORD_USER AND TBL_LOGIN.USER_NAME = _USER_NAME OR TBL_LOGIN.EMAIL_USER = _EMAIL_USER;
END$$
...
DELIMITER $$
USE `db_myinner`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_INSERT_USER`(_NOME_USER VARCHAR(15),_SOBRENOME_USER VARCHAR(15),_CPF VARCHAR(50),
_EMAIL_USER VARCHAR(255),_SEXO_USER CHAR,_PAIS_USER VARCHAR(100),_DATANASC_USER DATE,_ROLE_USER CHAR(1), _USER_NAME VARCHAR(255),_CHECKBOXCONTRATO TINYINT(1))
BEGIN
DECLARE _ID_USER INT;
SET _ID_USER = RETURN_ID_USER();
INSERT INTO TBL_USER (ID_USER,NOME_USER,SOBRENOME_USER,CPF,EMAIL_USER,SEXO_USER,PAIS_USER,DATANASC_USER,ROLE_USER,USER_NAME,CHECKBOXCONTRATO) 
VALUES (_ID_USER,_NOME_USER,_SOBRENOME_USER,_CPF,_EMAIL_USER,_SEXO_USER,_PAIS_USER,_DATANASC_USER,_ROLE_USER,_USER_NAME,_CHECKBOXCONTRATO);
END$$
